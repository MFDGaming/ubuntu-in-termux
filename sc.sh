...
if [ -d "$directory" ];then
first=1
printf "Skipping the download and the extraction\n"
elif [ -z "$(command -v proot)" ];then
printf "Please install proot.\n"
exit 1
...
*)
printf "Unknown architecture :- $ARCHITECTURE"
exit 1
;;
...
printf "Fixing the resolv.conf, so that you have access to the internet\n"
printf "nameserver 8.8.8.8\nnameserver 8.8.4.4\n" > etc/resolv.conf
stubs=()
stubs+=('usr/bin/groups')
for f in ${stubs[@]};do
printf "Writing stubs please wait...\n"
echo -e "#!/bin/sh\nexit" > "$f"
done
printf "Successfully wrote stubs!\n"
cd $cur
...
printf "Fixing shebang of startubuntu.sh please wait...\n"
termux-fix-shebang $bin
printf "Successfully fixed shebang of startubuntu.sh! \n"
